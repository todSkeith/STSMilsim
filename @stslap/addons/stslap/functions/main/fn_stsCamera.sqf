...
switch _mode do {
	...
	case "Init": {
		...
		with uinamespace do {
			BIS_fnc_guiEffectTiles_ppChromAberration ppEffectEnable false;
		};
	...
	case "Mouse": {
		...
		//if (BIS_fnc_camera_keys select DIK_LCONTROL) then {_coef = _coef / 10;};
	...
	case "KeyDown": {
		...
		switch (_key) do {
			...
			case (DIK_F1): {
				hint "This Function is Disabled";
...
*/
						...
						[] spawn
						{
							...
							while {true} do
							{
							   ...
							   
								{
									...
									_marker setMarkerTextLocal format["%1 (%2) - %3", name _x,_x,round (player distance _x)];
								   
									_markers set[count _markers,[_marker,_x]];
									switch (side _x) do
									{
										case west: {_marker setMarkerColorLocal "ColorBlue"};
										default {_marker setMarkerColorLocal "ColorRed"};
									};
	...
	case "Draw3D": {
		if (BIS_fnc_camera_visibleHUD) then {
			...
			onEachFrame
			{
				private["_vis","_color","_pos","_near"];
				_near = playableUnits;
				{
					if(_cam distance _x < 1500 && isPlayer _x && _x != _cam) then
					{
						_vis = true;
						if(true) then
						{
							...
							_color = [1,1,1,1];
							drawIcon3D ["",_color,_pos,0,0,0,format["%1 (%2) - %3m",name _x, _x, round (_cam distance _x)],0,0.04];
						};
					};
				} foreach _near;
			};
